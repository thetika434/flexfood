...
class _PageScannerQREtat extends State<PageScannerQR> {
  ...
  String? _extraireMatricule(String donneesQR) {
    final parties = donneesQR.split('-');
    if (parties.length < 5 || parties.first.toUpperCase() != 'FLEXFOOD') {
      return null;
    }

    final matricule = '${parties[1]}-${parties[2]}-${parties[3]}';
    final formatMatricule = RegExp(r'^\d{2}-[A-Z]+-\d{4}$');

    if (!formatMatricule.hasMatch(matricule)) {
      return null;
    }
